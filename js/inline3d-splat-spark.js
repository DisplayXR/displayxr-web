// inline3d-splat-spark.js — the three.js + Spark backend of `addSplat` (`engine: 'spark'`).
//
// EXPERIMENTAL. Internal to `./splat` (./inline3d-splat.js), which imports this module
// DYNAMICALLY and only when a page asks for `engine: 'spark'` (or falls back to it) — so a page
// on the default PlayCanvas backend never resolves `three` or `@sparkjsdev/spark`. Not covered by
// the SDK's 1.x semver promise — see docs/sdk-stability.md.
//
...
// OPTIONAL in package.json: the core SDK stays dependency-free and only pages that ask for this
// backend pay for them.
 ...
 */
export function attachSparkSplat(out, wall, canvas, src, opts = {}, pending = []) {
  // Fail FIRST, before any GPU object exists: a peer too old is an install-time mistake in the
  // page's dependencies, not a condition of this asset. Since this backend is loaded on demand the
  // throw arrives as a `ready` rejection (it used to be synchronous, from `addSplat` itself), with
  // a message naming the version problem rather than the asset.
  ...
  // Replay what the page called on the handle while this backend was loading — exclude() above
  // all, which a product page calls on the very next line after addSplat.
