...
| Call | What |
|---|---|
| `addSplat(…, { reveal })` | `'inflate' \| 'sweep' \| 'dissolve' \| 'fade' \| 'assemble' \| 'dissolve-in' \| 'converge' \| 'shimmer' \| false \| { type, durationMs, holdMs, easing, origin, …params }`. The effect is installed at its start state before the asset's first frame, with the splat hidden. It plays once `handle.firstWoven` settles (at once in 2D) and the first two frames have built, and the splat first shows on the frame its clock starts: the held start state is never on screen. |
